// Adapted from the wasm-pocket-tts demo worker in LaurentMazare/xn (Apache-2.0).
// The only changes are that the model, tokenizer, and voice URLs come in on a
// 'configure' message rather than being constants, so the game chooses where
// they live. See runtime/NOTICE.md.
...
async function handleLoad(quant) {
  if (!CONFIG.voices.length) throw new Error('no voices configured');
  const wasmModule = await wasmModulePromise;
  await init(wasmModule);
  post('status', { message: 'WASM initialized. Downloading tokenizer and model...' });

  const tokData = await fetchWithProgress(CONFIG.tokenizerUrl || TOKENIZER_URL, 'Tokenizer');
